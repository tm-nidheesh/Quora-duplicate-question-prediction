...
def preprocess(q):
    ...
    # The '[math]'appears around 900 times in the whole dataset and is of no use,so we will remove it.
    q = q.replace('[math]', '')
...
def predict():
    ...
    query = query_point_creator(question1,question2)
    result = model.predict(query)[0]
    if result == 1:
        result ='Question are duplicate'
    else:
        result ='Question are not duplicate'
